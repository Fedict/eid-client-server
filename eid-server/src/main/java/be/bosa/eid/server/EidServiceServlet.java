...
import be.bosa.eid.server.impl.handler.AuthSignResponseMessageHandler;
import be.bosa.eid.server.impl.handler.AuthenticationDataMessageHandler;
import be.bosa.eid.server.impl.handler.ClientEnvironmentMessageHandler;
import be.bosa.eid.server.impl.handler.ContinueInsecureMessageHandler;
import be.bosa.eid.server.impl.handler.FileDigestsDataMessageHandler;
import be.bosa.eid.server.impl.handler.HandlesMessage;
...
import be.bosa.eid.server.impl.handler.InitParam;
import be.bosa.eid.server.impl.handler.MessageHandler;
import be.bosa.eid.server.impl.handler.SignCertificatesDataMessageHandler;
 ...
 */
public class EidServiceServlet extends AbstractEidServiceServlet {

	private static final Log LOG = LogFactory.getLog(EidServiceServlet.class);
	...
	public void init(ServletConfig config) throws ServletException {
		...
		for (Class<? extends MessageHandler<?>> messageHandlerClass : MESSAGE_HANDLER_CLASSES) {
			...
			if (handlesMessageAnnotation == null) {
	...
	public static void injectInitParams(ServletConfig config, MessageHandler<?> messageHandler)
		...
		for (Field field : fields) {
			...
			if (ServiceLocator.class.equals(fieldType)) {
				 ...
				 */
				ServiceLocator<Object> fieldValue = new ServiceLocator<>(initParamName, config);
