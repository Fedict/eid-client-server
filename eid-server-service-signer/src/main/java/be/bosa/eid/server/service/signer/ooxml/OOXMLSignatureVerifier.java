...
import be.bosa.eid.server.service.signer.KeyInfoKeySelector;
import be.bosa.eid.server.service.signer.facets.XAdESXLSignatureFacet;
import be.bosa.eid.server.service.signer.jaxb.opc.contenttypes.CTDefault;
import be.bosa.eid.server.service.signer.jaxb.opc.contenttypes.CTOverride;
import be.bosa.eid.server.service.signer.jaxb.opc.contenttypes.CTTypes;
...
import be.bosa.eid.server.service.signer.jaxb.opc.relationships.STTargetMode;
...
public class OOXMLSignatureVerifier {
	...
	private boolean isIdPackageObjectValid(String signatureId, XMLObject idPackageObject, byte[] document)
		...
		return validateSignatureProperty((SignatureProperty) signatureProperties.getProperties().get(0), signatureId);
	}
	...
	private Map<String, String> getResources(byte[] document)
		...
		while (null != (zipEntry = zipInputStream.getNextEntry())) {
			...
			for (String contentType : OOXMLSignatureFacet.contentTypes) {
				NodeList nodeList = XPathUtil.getNodeListByXPath(contentTypesDocument, nsElement, "/tns:Types/tns:Override[@ContentType='" + contentType + "']/@PartName");
