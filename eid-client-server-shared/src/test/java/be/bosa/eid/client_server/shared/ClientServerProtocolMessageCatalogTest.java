...
public class ClientServerProtocolMessageCatalogTest {

	private static final Log LOG = LogFactory.getLog(ClientServerProtocolMessageCatalogTest.class);
	...
	private void generateDocbook(File tmpFile) throws FileNotFoundException, IllegalAccessException {
		PrintWriter writer = new PrintWriter(tmpFile);
		writer.println("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
		writer.println("<!DOCTYPE section PUBLIC ");
		writer.println("\"-//OASIS//DTD DocBook XML V4.5//EN\"");
		writer.println("\"http://www.oasis-open.org/docbook/xml/4.5/docbookx.dtd\">");
		writer.println("<section>");
		writer.println("<title>eID Server Protocol Messages</title>");
		writer.println("<para>The following documentation has been generated automatically.</para>");
		writer.println("<!-- Autogenerated by " + ClientServerProtocolMessageCatalogTest.class.getSimpleName() + " -->");

		ClientServerProtocolMessageCatalog catalog = new ClientServerProtocolMessageCatalog();
	...
	private void describeClass(Class<?> catalogClass, PrintWriter writer)
		...
		if (null != startRequestMessage) {
			writer.println("<para>");
			writer.println("This message starts a communication session between eID Client and eID Server Service.");
		...
		if (null != stopResponseMessage) {
			writer.println("<para>");
			writer.println("This message stops a communication session between eID Client and the eID Server Service.");
			writer.println("</para>");
		}
		...
		if (null != protocolStateAllowed) {
			writer.println("<para>");
			writer.println("This message is only accepted if the eID Server Service protocol state is: "
		...
		if (null != responsesAllowedAnnotation) {
			...
			writer.println("<para>");
			writer.println("Allowed eID Server Service response messages are: ");
		...
		if (null != stateTransition) {
			writer.println("<para>");
			writer.println(
					"This message will perform an eID Server protocol state transition to: " + stateTransition.value());
			writer.println("</para>");
		}
